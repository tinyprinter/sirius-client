const CONNECT = (bridgeAddress: string): object => {
  return {
    type: 'BridgeEvent',
    json_payload: {
      ncp_version: '0x46C5',
      uptime: '45.71 23.94',
      firmware_version: 'v2.3.1-f3c7946',
      network_info: {
        extended_pan_id: '0x42455247fbbbbd7f',
        node_type: 'EMBER_COORDINATOR',
        radio_power_mode: 'EMBER_TX_POWER_MODE_BOOST',
        security_level: 5,
        network_status: 'EMBER_JOINED_NETWORK',
        channel: 11,
        security_profile: 'Custom',
        power: 8,
        node_eui64: '0x000d6f0001b3719d',
        pan_id: '0xDF3A',
        node_id: '0x0000',
      },
      name: 'power_on',
      local_ip_address: '192.168.1.98',
      uboot_environment:
        '',
      mac_address: '40:d8:55:01:c1:b3',
      model: 'A',
    },
    bridge_address: bridgeAddress,
    timestamp: 1426256447.70695,
  };
};

const ENCRYPTION_KEY_REQUIRED = (
  bridgeAddress: string,
  deviceAddress: string
): object => {
  return {
    timestamp: 1419107228.91187,
    type: 'BridgeEvent',
    json_payload: {
      name: 'encryption_key_required',
      device_address: deviceAddress,
    },
    bridge_address: bridgeAddress,
  };
};

const HEARTBEAT = (bridgeAddress: string, deviceAddress: string): object => {
  return {
    rssi_stats: [-20, -20, -19],
    device_address: deviceAddress,
    binary_payload: 'AQAAAAAABAAAAG4AAAA=',
    type: 'DeviceEvent',
    bridge_address: bridgeAddress,
    timestamp: 1426256547.52687,
  };
};

export { CONNECT, ENCRYPTION_KEY_REQUIRED, HEARTBEAT };
