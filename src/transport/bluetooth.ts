...
let bluetooth = new BluetoothSerialPort();
let device = null;
let connection = null;
...
class Bluetooth extends EventEmitter {
  ...
  constructor(address: string, channel: number) {
    ...
    device = new BluetoothSerialPort();
  }

  async findPrinters(): Promise<
    { address: string; name: string; channel: number } | undefined
  > {
    const devices = await device.scan();
    const printers = await Promise.all(devices.map());
    return undefined;
  }

  async getDevice(address: string, channel: number): Promise<Bluetooth> {
